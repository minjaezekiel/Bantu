...
- **[feature]** `bp_text(canvas, x, y, text, size, colour, anchor, rotate, opacity)` — the SVG
  backend's `text()` arguments, so the raster backend can be a drop-in. `(x, y)` is the start of the
  baseline, the anchor is `start`/`middle`/`end`, and rotation is degrees clockwise about `(x, y)`.
- **[feature]** `bp_text_width(text, size)` — the advance in user units, measured in the font the PNG
  draws with. It uses the same UTF-8 decoder as drawing, so the two agree on every input.
- **[feature]** The font is **DejaVu Sans, 210 glyphs, 4,105 outline segments** (`raster_font.hpp`,
  generated by `scripts/gen_font_tables.py`, licence carried in the header). Curves are flattened by
  exact integer Bernstein weights and the string is one coverage mask; positions are summed in font
  units and scaled once per point, so long labels do not drift.
- **[feature]** Rotation snaps to the 256-step circle table already embedded for arcs (1.4°): 0°, 45°
  and 90° are exact, and nothing calls `cos`. CORDIC is the documented upgrade if an exact arbitrary
  angle is ever wanted.
- **[feature]** Strict UTF-8: stray, truncated, overlong and surrogate sequences become `U+FFFD`, one
  per bad byte, and characters the font lacks draw as `U+FFFD`. Limits: 2,000 characters, a
  4,096-pixel font.
- **Speed**: 0.13 ms per six-character tick label at 96 dpi, 0.25 ms at 300 dpi.

### Solved defects encountered

- **[bug fix] Accented letters rendered blank.** `ö ç à ÿ` and most of Latin-1 are *composite*
  glyphs in TrueType — a base letter plus an accent — and the generator's recording pen returned them
  as `addComponent` operations, which its loop silently dropped. Found by looking at the first rendered
  sample. The generator now decomposes components (1,366 more segments), and **raises on any pen
  operation it does not handle** instead of skipping it; the test now requires every one of the 207
  inked glyphs to draw its own shape rather than `U+FFFD`, and `ö` to differ from `o`.
- **[bug fix] Coordinates far off the canvas overflowed 64-bit arithmetic (a B6c defect).** `centi`
  allows ±10⁸ user units, about 2⁴⁰ in Q24.8 at 2,400 dpi; the clip crossing multiplied two such spans
  and a stroke squared its length. UBSan reported **four** overflow sites — the crossing, the stroke
  normal and two in the dash walker — none of which the B6c fuzz reached, because it stayed within
  twice the canvas. The documentation claimed no coordinate could overflow; it was wrong and is
  corrected. Fixed at the root, without `__int128` (the Windows build is MSVC): a crossing uses the
  exact rational when the product fits and **bisects** otherwise; strokes are clipped to a guard box
  half a million pixels beyond the canvas before anything is squared; stroke width is capped at 65,536
  px; an arc spanning more than 2²⁹ Q8 draws as its chord. On the pre-fix binary the new assertions
  fail 2 of 5 — a line to a point 10⁸ away bent toward the wrong corner, and a 10⁸-wide triangle
  filled nothing. Every existing PNG is byte-identical.
- **[bug fix] The font generator lost any contour with no on-curve point.** A TrueType contour made
  only of off-curve points (fontTools marks it with a trailing `None`) produced segments with no
  starting move. DejaVu has none, so no output changed, but another font would have drawn garbage.

### Tests

- **[test]** `tests/bplot_raster_test.b` — **142 assertions** (106 before): every embedded glyph inks
  and draws itself; invalid and missing characters are `U+FFFD`; exact DejaVu advances; middle and end
  anchors and the baseline land where the metrics say; −90° runs up from its anchor; 360° is no
  rotation; opacity, clip and `none`; byte-identical repeat renders; every limit raising by name; and
  the far-coordinate cases above.
